import { RgbDAOABI, RgbDaoLogicV1Factory } from '@rgbdao/sdk';
...
const abi = new utils.Interface(RgbDAOABI);
const rgbDaoContract = new RgbDaoLogicV1Factory().attach(config.addresses.rgbDAOProxy);
const proposalCreatedFilter = rgbDaoContract.filters?.ProposalCreated(
  null,
  null,
  null,
  null,
...
export const useHasVotedOnProposal = (proposalId: string | undefined): boolean => {
  ...
  const [receipt] =
    useContractCall<[any]>({
      abi,
      address: rgbDaoContract.address,
      method: 'getReceipt',
...
const useVotingDelay = (rgbDao: string): number | undefined => {
  const [blockDelay] =
    useContractCall<[EthersBN]>({
      abi,
      address: rgbDao,
...
export const useAllProposals = (): ProposalData => {
  ...
  const votingDelay = useVotingDelay(rgbDaoContract.address);
...
export const usePropose = () => {
  const { send: propose, state: proposeState } = useContractFunction(rgbDaoContract, 'propose');
