import { Proposal } from '../../wrappers/rgbDao';
...
import { ProposalState } from '../../wrappers/rgbDao';

import classes from './RgbProfileVoteRow.module.css';

import { useHistory } from 'react-router-dom';
import { useQuery } from '@apollo/client';
import { highestRgbIdMintedAtProposalTime } from '../../wrappers/subgraph';
import VoteStatusPill from '../VoteStatusPill';

import _PendingVoteIcon from '../../assets/icons/PendingVote.svg';
import { Vote } from '../../utils/vote';
import { RgbVoteHistory } from '../ProfileActivityFeed';

interface RgbProfileVoteRowProps {
  proposal: Proposal;
  rgbId: number;
...
const selectIconForRgbVoteActivityRow = (proposal: Proposal, vote?: RgbVoteHistory) => {
  if (!vote) {
...
const selectVotingInfoText = (proposal: Proposal, vote?: RgbVoteHistory) => {
  if (!vote) {
    if (proposal.status === ProposalState.PENDING || proposal.status === ProposalState.ACTIVE) {
      return 'Waiting for';
    }
...
const RgbProfileVoteRow: React.FC<RgbProfileVoteRowProps> = props => {
  const { proposal, vote, rgbId, latestProposalId } = props;

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { loading, error, data } = useQuery(highestRgbIdMintedAtProposalTime(proposal.startBlock));
  ...
  // In this case, rgb was not yet minted at time of proposal
  if (data && data.auctions.length > 0 && rgbId > data.auctions[0].id) {
    if (proposal.id === latestProposalId.toString()) {
      return (
        <tr className={classes.nullStateCopy}>This Rgb has no activity yet. Check back soon!</tr>
      );
    }
    return <></>;
  }
  ...
  return (
    <tr onClick={proposalOnClickHandler} className={classes.voteInfoRow}>
      <td className={classes.voteIcon}>{selectIconForRgbVoteActivityRow(proposal, vote)}</td>
      <td>
...
export default RgbProfileVoteRow;
