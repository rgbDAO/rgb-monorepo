import { Auction, AuctionHouseContractFunction } from '../../wrappers/rgbAuction';
...
import { useAuctionMinBidIncPercentage } from '../../wrappers/rgbAuction';
...
const Bid: React.FC<{
  ...
  const rgbAuctionHouseContract = new RgbAuctionHouseFactory().attach(
  ...
  const placeBidHandler = async () => {
    ...
    const contract = connectContractToSigner(rgbAuctionHouseContract, undefined, library);
    const gasLimit = await contract.estimateGas.createBid(auction.rgbId, {
      value,
    });
    placeBid(auction.rgbId, {
      value,
  ...
  const fomoRgbBtnOnClickHandler = () => {
    // Open Fomo Rgb in a new tab
    ...
    <>
      ...
      <InputGroup>
        ...
        {!auctionEnded ? (
          ...
          <>
            <Button className={classes.bidBtnAuctionEnded} onClick={fomoRgbBtnOnClickHandler}>
              Vote for the next RGB ⌐◧-◧
            </Button>
