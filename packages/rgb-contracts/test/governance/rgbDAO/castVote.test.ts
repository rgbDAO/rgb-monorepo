...
import {
  ...
  RgbDaoLogicV1Harness,
  RgbDaoLogicV1Harness__factory as RgbDaoLogicV1HarnessFactory,
} from '../../../typechain';
...
async function deployGovernor(
  ...
  tokenAddress: string,
): Promise<RgbDaoLogicV1Harness> {
  const { address: govDelegateAddress } = await new RgbDaoLogicV1HarnessFactory(
    deployer,
  ).deploy();
  const params = [
    address(0),
  ...
  const { address: _govDelegatorAddress } = await (
    await ethers.getContractFactory('RgbDAOProxy', deployer)
  ).deploy(...params);

  return RgbDaoLogicV1HarnessFactory.connect(_govDelegatorAddress, deployer);
}
...
async function reset() {
  ...
  token = await deployRgbToken(signers.deployer);

  await populateDescriptor(
    RgbDescriptorFactory.connect(await token.descriptor(), signers.deployer),
  );
  ...
  describe('We must revert if:', () => {
    ...
    it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
      await expect(gov.castVote(proposalId, 1)).revertedWith(
        'RgbDAO::castVoteInternal: voting is closed',
      );
    });
    ...
    it('Such proposal already has an entry in its voters set matching the sender', async () => {
      ...
      await expect(gov.connect(account0).castVote(proposalId, 1)).revertedWith(
        'RgbDAO::castVoteInternal: voter already voted',
      );
    });
  });
