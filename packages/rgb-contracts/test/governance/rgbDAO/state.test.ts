...
import {
  ...
  RgbDaoExecutorHarness,
  RgbDaoExecutorHarness__factory as RgbDaoExecutorHarnessFactory,
  RgbDaoImmutable,
  RgbDaoImmutable__factory as RgbDaoImmutableFactory,
} from '../../../typechain';
...
let gov: RgbDaoImmutable;
let timelock: RgbDaoExecutorHarness;
let delay: number;
...
async function makeProposal(
  ...
  timelock = await new RgbDaoExecutorHarnessFactory(deployer).deploy(deployer.address, delay);

  gov = await new RgbDaoImmutableFactory(deployer).deploy(
    timelock.address,
    token.address,
    address(0),
    deployer.address,
...
describe('RgbDAO#state/1', () => {
  ...
  it('Invalid for proposal not found', async () => {
    await makeProposal();
    await expect(gov.state(5)).revertedWith('RgbDAO::state: invalid proposal id');
  });
