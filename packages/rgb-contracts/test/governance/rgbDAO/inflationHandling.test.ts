...
async function reset(): Promise<void> {
  ...
  const { address: timelockAddress } = await new RgbDaoExecutorFactory(deployer).deploy(
    govDelegatorAddress,
    timelockDelay,
  );
...
describe('RgbDAO#inflationHandling', () => {
  before(async () => {
  ...
  it('returns quorum votes and proposal threshold based on Rgb total supply', async () => {
    // Total Supply = 40
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    ).revertedWith('RgbDAO::propose: proposer votes below proposal threshold');
  });
