...
async function reset(): Promise<void> {
  ...
  // nonce 3: Deploy RgbDescriptor
  // nonce 4: Deploy RgbSeeder
  // nonce 5: Deploy RgbToken
  // nonce 6: Deploy RgbDAOProxy
  ...
  // Deploy RgbDAOExecutor with pre-computed Delegator address
  timelock = await new RgbDaoExecutorFactory(deployer).deploy(govDelegatorAddress, timelockDelay);
  const timelockAddress = timelock.address;

  // Deploy Delegate
  const { address: govDelegateAddress } = await new RgbDaoLogicV1Factory(deployer).deploy();

  // Deploy Rgb token
  token = await deployRgbToken(deployer);

  // Deploy Delegator
  await new RgbDaoProxyFactory(deployer).deploy(
    timelockAddress,
    token.address,
    vetoer.address,
    timelockAddress,
  ...
  gov = RgbDaoLogicV1Factory.connect(govDelegatorAddress, deployer);

  await populateDescriptor(RgbDescriptorFactory.connect(await token.descriptor(), deployer));
...
describe('RgbDAO#vetoing', () => {
  ...
  it('only vetoer can veto', async () => {
    ...
    await expect(gov.veto(proposalId)).revertedWith('RgbDAO::veto: only vetoer');
  });

  it('burns veto power correctly', async () => {
    // vetoer is still set
    expect(await gov.vetoer()).to.equal(vetoer.address);
    await expect(gov._burnVetoPower()).revertedWith('RgbDAO::_burnVetoPower: vetoer only');
    // burn
  ...
  describe('vetoing works correctly for proposal state', async () => {
    ...
    it('Executed', async () => {
      ...
      await expect(gov.veto(proposalId)).revertedWith(
        'RgbDAO::veto: cannot veto executed proposal',
      );
    });
    it('Vetoed', async () => {
      await propose(account0);
