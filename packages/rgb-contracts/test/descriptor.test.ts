...
import { RgbDescriptor } from '../typechain';
import ImageData from '../files/image-data.json';
import { LongestPart } from './types';
import { deployRgbDescriptor, populateDescriptor } from './utils';
...
describe('RgbDescriptor', () => {
  let rgbDescriptor: RgbDescriptor;
  ...
  before(async () => {
    rgbDescriptor = await deployRgbDescriptor();
    ...
    await populateDescriptor(rgbDescriptor);
  });
  ...
  it('should generate valid token uri metadata when data uris are disabled', async () => {
    const BASE_URI = 'https://api.rgbdao.xyz/metadata/';

    await rgbDescriptor.setBaseURI(BASE_URI);
    await rgbDescriptor.toggleDataURIEnabled();

    const tokenUri = await rgbDescriptor.tokenURI(0, {
      background: 0,
  ...
  it('should generate valid token uri metadata when data uris are enabled', async () => {
    ...
    expect(name).to.equal('Rgb 0');
    expect(description).to.equal('Rgb 0 is a member of the Rgb DAO');
  ...
  it.skip('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      const tokenUri = await rgbDescriptor.tokenURI(i, {
      ...
      expect(name).to.equal(`Rgb ${i}`);
      expect(description).to.equal(`Rgb ${i} is a member of the Rgb DAO`);
      expect(image).to.not.be.undefined;
