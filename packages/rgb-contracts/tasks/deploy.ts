import { default as RgbAuctionHouseABI } from '../abi/contracts/RgbAuctionHouse.sol/RgbAuctionHouse.json';
...
type ContractName =
  ...
  | 'RgbToken'
  | 'RgbAuctionHouse'
  | 'RgbAuctionHouseProxyAdmin'
  | 'RgbAuctionHouseProxy'
  | 'RgbDAOExecutor'
  | 'RgbDAOLogicV1'
  | 'RgbDAOProxy';
...
task('deploy', 'Deploys NFTDescriptor, RgbDescriptor, RgbSeeder, and RgbToken')
  .addParam('rgbfoundersdao', 'The rgbfounders DAO contract address', undefined, types.string)
  ...
  .setAction(async (args, { ethers }) => {
    ...
    const expectedRgbDAOProxyAddress = ethers.utils.getContractAddress({
      from: deployer.address,
    ...
    const contracts: Record<ContractName, Contract> = {
      ...
      RgbToken: {
        args: [
          ...
          () => contracts['RgbDescriptor'].address,
          () => contracts['RgbSeeder'].address,
      ...
      RgbAuctionHouseProxy: {
        args: [
          () => contracts['RgbAuctionHouse'].address,
          () => contracts['RgbAuctionHouseProxyAdmin'].address,
          () =>
            new Interface(RgbAuctionHouseABI).encodeFunctionData('initialize', [
              contracts['RgbToken'].address,
              args.weth,
      ...
      RgbDAOExecutor: {
        args: [expectedRgbDAOProxyAddress, args.timelockDelay],
      },
      RgbDAOLogicV1: {
        waitForConfirmation: true,
      },
      RgbDAOProxy: {
        args: [
          () => contracts['RgbDAOExecutor'].address,
          () => contracts['RgbToken'].address,
          args.rgbfoundersdao,
          () => contracts['RgbDAOExecutor'].address,
          () => contracts['RgbDAOLogicV1'].address,
          args.votingPeriod,
          args.votingDelay,
