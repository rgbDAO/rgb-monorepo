...
task('deploy-local', 'Deploy contracts to hardhat')
  ...
  .setAction(async (args, { ethers }) => {
    ...
    const contracts: Record<ContractName, Contract> = {
      ...
      RgbToken: {
        args: [
          args.rgbfoundersdao || deployer.address,
          expectedAuctionHouseProxyAddress,
          () => contracts['RgbDescriptor'].instance?.address,
          () => contracts['RgbSeeder'].instance?.address,
      ...
      RgbAuctionHouseProxy: {
        args: [
          () => contracts['RgbAuctionHouse'].instance?.address,
          () => contracts['RgbAuctionHouseProxyAdmin'].instance?.address,
          () =>
            new Interface(RgbAuctionHouseABI).encodeFunctionData('initialize', [
              contracts['RgbToken'].instance?.address,
      ...
      RgbDAOProxy: {
        args: [
          () => contracts['RgbDAOExecutor'].instance?.address,
          () => contracts['RgbToken'].instance?.address,
          args.rgbfoundersdao || deployer.address,
          () => contracts['RgbDAOExecutor'].instance?.address,
          () => contracts['RgbDAOLogicV1'].instance?.address,
          args.votingPeriod,
          args.votingDelay,
