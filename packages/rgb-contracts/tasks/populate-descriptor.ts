...
task('populate-descriptor', 'Populates the descriptor with color palettes and Rgb parts')
  .addOptionalParam(
    'nftDescriptor',
  ...
  .addOptionalParam(
    'rgbDescriptor',
    'The `RgbDescriptor` contract address',
    '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512',
    types.string,
  )
  .setAction(async ({ nftDescriptor, rgbDescriptor }, { ethers }) => {
    const descriptorFactory = await ethers.getContractFactory('RgbDescriptor', {
      libraries: {
        NFTDescriptor: nftDescriptor,
      },
    });
    const descriptorContract = descriptorFactory.attach(rgbDescriptor);
